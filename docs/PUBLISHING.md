...
> **まず無料の対処で足ります。** 配布は `scripts/install-mac.sh`（`curl` で
> 取得するため隔離属性が付かない）を案内すれば、未署名のままでも普通に起動
> できます。詳細は [`MACOS-INSTALL.md`](MACOS-INSTALL.md)。
> 以下は「配布先が増えてきて、ダブルクリックで入る状態にしたい」場合の手順です。
